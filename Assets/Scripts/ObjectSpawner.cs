...
public class ObjectSpawner : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        // Check if we have any spawnable objects
        if (spawnableObjects == null || spawnableObjects.Count == 0)
        {
            Debug.LogWarning("No object types assigned to ObjectSpawner! Please add at least one.");
        }
    }
    ...
    {
        if (spawnableObjects != null && spawnableObjects.Count > 0 &&
            Time.time >= nextSpawnTime && currentObjectsCount < maxObjectsOnScreen)
        {
            SpawnObject();
            nextSpawnTime = Time.time + currentSpawnRate;
